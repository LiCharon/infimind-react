...
以下问题只在当前 Windows 开发机上出现，与业务代码无关，但会显著影响开发效率。

### 10.1 `.git/refs` 下新建的 ref 目录会被清空

**症状**：`git commit` 报告成功，但紧接着 `git log` 报 `does not have any commits yet`，`git push` 报 `src refspec <branch> does not match any`。

**原因**：`.git/refs/heads/<new-branch>` 与 `.git/refs/remotes/origin/` 这类**新建**目录会被整个删除；原有的 `heads/main`、`remotes/upstream/` 不受影响。已多次复现，规律稳定。commit 对象本身是完好的。

**处理流程（已验证有效）**：

```bash
# 1. 确认 commit 对象还在
git cat-file -t <短hash>        # 应输出 commit
git rev-parse <短hash>          # 取完整 40 位 hash

# 2. 手工重写 ref 文件（用 Node 写，写入不会被拦）
#    .git/refs/heads/<branch>  内容为完整 hash + 换行

# 3. 再 push
git push origin <branch>
```

直接手工重建整个 refs 目录也是可行的（`mkdir` + 写文件都正常）。恢复后用 `git fsck`（静默即无错误）与 `git rev-parse HEAD` 验证。

### 10.2 中文路径与命令行工具

仓库位于 `E:\桌面\资料\法智科讯\法飞飞\infimind-react`。**命令行工具传中文路径会乱码**（PowerShell `Get-ChildItem`、`git -C` 等会拿到 `E:\妗岃潰\...` 这类错误路径而失败），但 Node 的 `fs` / `path` API 能正确处理。

**绕过办法**：创建纯 ASCII 目录联接指向仓库，再对链接路径执行 git 命令。

```cmd
mklink /J C:\Users\<user>\_imlink "E:\桌面\资料\法智科讯\法飞飞\infimind-react"
git -C C:\Users\<user>\_imlink status
```

### 10.3 后台服务需用常驻方式启动

用 `spawn({detached:true}) + unref()` 方式启动的 API / Worker 进程曾出现静默消失（无日志、无退出码，端口无人监听）。前端此时会报「暂时无法恢复登录状态」——该文案来自 `src/utils/auth-api.js` 的 `AuthNetworkError`，**只在 `fetch` 抛异常时出现**；服务端正常返回 401 时前端是静默未登录态、不报错，可据此区分「服务挂了」和「会话过期」。

改用前台常驻方式挂住进程即可稳定运行。

### 10.4 Playwright 浏览器版本错配

本机已安装 `%LOCALAPPDATA%\ms-playwright\chromium-1228`，但 Playwright 1.63 默认寻找 `chromium_headless_shell-1243`，会报「Executable doesn't exist」。**不要执行 `npx playwright install`**（会触发下载），改为显式指定已有可执行文件：

```js
chromium.launch({ executablePath: '<...>/chromium-1228/chrome-win64/chrome.exe' })
```

另：Vite 开发服务器存在 HMR WebSocket 长连接，`waitUntil: 'networkidle'` 会一直超时，应改用 `domcontentloaded`。
